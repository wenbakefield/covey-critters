...
While a user is in a town, communication between frontend and backend for SpaceBarGame occurs primiarily over socket API. The events that shared over the socket protocol are defined in `shared/types/CoveyTownSocket.d.ts`:

```mermaid
sequenceDiagram
    participant BC as Town (Backend)
    participant AC as Avery's TownController
    participant CC as useSpaceBarGame Hook
    AC->>BC: initializeCarnivalGame(gameState)
    AC->>CC: gameChanged
    CC->>AC: emitGameOnTick(key)
    AC->>BC: updateGame(key)
    BC->>AC: gameUpdated(SpaceBarGame)
    AC->>CC: gameChanged
    AC->>BC: timeLimitReach()
    BC->>AC:gameUpdated(SpaceBarGame)
    AC->>CC: gameTimeOut
```

While a user is in a town, communication between frontend and backend for PetMovement occurs primiarily over socket API. The events that shared over the socket protocol are defined in `shared/types/CoveyTownSocket.d.ts`:

```mermaid
sequenceDiagram
    participant BC as Town (Backend)
    participant AC as Avery's Pet/Player Controller
    participant CC as TownGameScene
    AC->>BC: assignPetToPlayer()
    AC->>CC: petChange
    CC->>CC: add Avery's pet sprite to 2D Map
    AC->>BC: emitMovement(Avery's playerLocation)
    BC->>AC: petMoved(PetOwnerMap)
    AC->>CC: petMovementChange
    CC->>CC: update Avery's pet location
```
